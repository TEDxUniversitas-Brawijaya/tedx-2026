...
const app = await alchemy("tedx-2026-dashboard", {
  stateStore:
...
const worker = await Vite("dashboard", {
  name: `tedx-2026-dashboard-${app.stage}`,
  bindings: {
    VITE_PUBLIC_API_URL: process.env.API_URL || "http://localhost:3000",
  },
  domains:
    app.stage === "prod" ? ["dash.tedxuniversitasbrawijaya.com"] : undefined,
...
if (process.env.PULL_REQUEST) {
  ...
  await GitHubComment("preview-comment", {
    ...
    body: `### Preview Dashboard Deployment
