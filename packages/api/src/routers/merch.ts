...
const getDummyMerchProducts = () => {
  const now = new Date();
  return listMerchProductsOutputSchema.parse([
    // -- Merch Regular --
    {
      id: "prod_m_tshirt_1",
      type: "merch_regular",
          ...
          products: [
            {
              ...
              variants: [{ id: "var_kc_std", type: "size", label: "Standard" }],
            },
          ],
        },
      ],
...
const getDummyMerchPrice = (productId: string) => {
  const product = getDummyMerchProducts().find((item) => item.id === productId);
  if (!product) {
    throw new Error(`Unknown merch productId: ${productId}`);
  }
