...
import { ticketsTable, type SelectTicket } from "../schemas/tickets";
...
export const createAttendanceQueries = (db: DB): AttendanceQueries => ({
  ...
  checkInTicket: async (ticketId, checkedInBy, checkedInAt) => {
    const [ticket] = await db
      .update(ticketsTable)
      .set({
      ...
      .where(
        and(
          ...
          eq(ticketsTable.attendanceStatus, "not_checked_in")
        )
      )
      .returning();
